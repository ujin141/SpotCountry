...
const addTourDescription = async () => {
  const sql = `
    -- Add description to tours table
    do $$ 
    begin 
      if not exists (select 1 from information_schema.columns where table_name = 'tours' and column_name = 'description') then
        alter table tours add column description text;
      end if;
    end $$;
  `;

  console.log("Updating tours schema...");
  const { error } = await supabase.rpc('exec_sql', { query: sql });
  
  if (error) {
    console.error("Error updating schema:", error);
  } else {
    console.log("Schema updated! 'description' column added to tours.");
  }
};
